...
(function () {
  ...
  async function loadUser() {
    // Tenta a API; usa cache do localStorage se o banco foi resetado (/tmp no Vercel)
    let user;
    try {
      const data = await api('/api/auth/me');
      user = data.user;
    } catch (err) {
      // 401 ja trata redirecionamento em api(). Para 404/500 usa dados em cache.
      ...
      if (!user) {
        console.error(err);
        toast('Nao foi possivel carregar seus dados. Tente novamente.', 'error');
        return;
      }
    }
    if (!user) {
    ...
    var bname = document.getElementById('business-name');
    if (uname) uname.textContent = user.name || '-';
    if (bname) bname.textContent = user.business_name || user.name || '-';
  }
